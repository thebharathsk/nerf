...
class SinusoidalEmbeddings(nn.Module):
    def __init__(self, embedding_config):
        ...
        #create embeddings
        self.num_freq = embedding_config['num_freq']
    ...
    def forward(self, coords):
        ...
        """        
        #get frequencies
        freqs = torch.Tensor([2**i for i in range(self.num_freq)]).unsqueeze(0).unsqueeze(0).to(coords.device) #1x1xF
        
        #increase dimensionality of coords
        coords = coords.unsqueeze(2) #Bx2x1
        
        #get embeddings
        embeddings_sin = torch.sin(freqs*math.pi*coords) #Bx2xF
        embeddings_cos = torch.cos(freqs*math.pi*coords) #Bx2xF
